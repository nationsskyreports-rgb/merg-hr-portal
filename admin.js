...
// ✅ FIX: بيبني الـ sidebar بس ومش بيعيد كل حاجة
...
// ── دالة مشتركة لتحميل الـ dropdowns ──
async function loadEmpDropdowns(selectedDept='', selectedPos='') {
  ...
  if(deptSel) {
    deptSel.innerHTML = `<option value="">${lang==='ar'?'اختر القسم':'Select Department'}</option>` +
      deptList.map(d=>`<option value="${d}" ${d===selectedDept?'selected':''}>${d}</option>`).join('');
  }
  if(posSel) {
    posSel.innerHTML = `<option value="">${lang==='ar'?'اختر المنصب':'Select Position'}</option>` +
      posList.map(p=>`<option value="${p}" ${p===selectedPos?'selected':''}>${p}</option>`).join('');
  }
}
...
async function handleSaveEmployee() {
  ...
  let error;
  if(editingEmpId) {
    ({error} = await sb.from('employees').update(payload).eq('id', editingEmpId));
  } else {
    ...
    <div style="position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,.55);backdrop-filter:blur(6px);z-index:2000;display:flex;align-items:center;justify-content:center;padding:16px">
      <div style="background:var(--surface);border-radius:var(--r-2xl);padding:28px 24px;width:90%;max-width:400px;border:1px solid var(--border);box-shadow:var(--shadow-lg)">
        ...
        <div style="display:flex;gap:10px;margin-top:10px">
          ...
          <button onclick="saveNewOption('${type}')" class="primary-btn">${lang==='ar'?'إضافة':'Add'}</button>
        </div>
      </div>
    </div>
  `;
...
async function saveNewOption(type) {
  ...
  // حفظ في app_settings مش في جدول منفصل عشان يتزامن مع loadEmpDropdowns
          ...
          return `<div class="card-sm" style="display:flex;align-items:center;gap:12px">
            ...
            <div style="display:flex;gap:8px;flex-shrink:0">
              <button onclick="downloadFileSecurely('${f.id}','${f.file_path||f.file_url}')"
                      style="background:rgba(56,189,248,.12);border:none;border-radius:8px;width:34px;height:34px;display:flex;align-items:center;justify-content:center;cursor:pointer;font-size:16px">👁</button>
              <button onclick="deleteFile('${f.id}','${f.file_path||f.file_url}')"
                      style="background:rgba(239,68,68,.12);border:none;border-radius:8px;width:34px;height:34px;cursor:pointer;color:var(--red);font-size:14px">✕</button>
            </div>
          </div>`;
        }).join('')}
  `;
  ...
  showConfirm({
    ...
    onOk:async()=>{
      if(isDeductible) {
        ...
        await sb.from('employees').update({leave_balance:(emp?.leave_balance??0)+days}).eq('id',empId);
      }
...
// ✅ FIX: renderCustomPage بتعرف هي في admin mode ولا employee mode
