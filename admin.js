...
const FILE_CONFIG = {
  ...
  isValid: function(fileType, fileSizeInBytes) {
    return this.ALLOWED_MIME_TYPES.includes(fileType) && fileSizeInBytes <= (this.MAX_SIZE_MB * 1024 * 1024);
  }
};
...
function showHRTab(tab) {
  ...
  document.querySelectorAll('#hrApp button[onclick^="showHRTab"]').forEach(btn => {
    const oc = btn.getAttribute('onclick') || '';
    const isActive = oc.includes(`'${tab}'`);
    btn.style.fontWeight = isActive ? '800' : '600';
    btn.style.background = isActive ? 'var(--indigo-dim)' : 'transparent';
    btn.style.color      = isActive ? 'var(--indigo)' : 'var(--muted)';
    btn.style.outline    = isActive ? '1.5px solid var(--indigo-mid)' : 'none';
  });
  ...
  $('hrContent').innerHTML = `
    ...
    ${late.length > 0 ? `
      ...
      ${late.map(a => {
        ...
        const checkDate = new Date(`1970-01-01T${a.check_in_time}`);
        const mins = Math.round((checkDate - shiftDate) / 60000);
...
async function renderHRAttendance() {
  ...
  const {data:att, error:attErr} = await sb.from('attendance_records').select('*').eq('attendance_date',safeDate).order('check_in_time',{ascending:false});
  if(attErr) { $('hrContent').innerHTML = `<div class="empty"><div class="empty-icon">⚠️</div><div class="empty-title">${attErr.message}</div></div>`; return; }
  const items = att||[];
  let empMap = {};
  if(items.length > 0) {
  ...
  $('hrContent').innerHTML = `
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px;flex-wrap:wrap;gap:10px">
      ...
      <input type="date" class="form-input" style="width:auto;padding:8px 12px" value="${safeDate}" onchange="attDate=this.value||'${safeDate}';renderHRAttendance()"/>
    </div>
    ${items.length===0
  ...
  $('hrContent').innerHTML = `
    ...
    ${items.length===0
      ...
      :items.map(lv=>`<div class="leave-card" style="border-color:${lv.status==='pending'?'rgba(245,158,11,.35)':''}">
          ...
          ${lv.status==='pending'?`
            <div style="display:flex;gap:8px">
              <button class="primary-btn" id="approve_${lv.id}" style="flex:1;background:var(--green);padding:8px" onclick="approveLeave('${lv.id}')">${t().approve}</button>
              <button class="primary-btn" id="reject_${lv.id}" style="flex:1;background:var(--red);padding:8px" onclick="rejectLeave('${lv.id}')">${t().reject}</button>
...
async function approveLeave(id) {
  ...
  if(DEDUCTIBLE.includes(lv.leave_type?.toLowerCase())) {
    ...
    await sb.from('employees').update({leave_balance:Math.max(0,(emp?.leave_balance??21)-days)}).eq('id',lv.employee_id);
  }
...
async function loadEmpDropdowns(selectedDept='', selectedPos='') {
  ...
  const deptSel = $('ae_dept_select');
  const posSel  = $('ae_pos_select');
  if(deptSel) deptSel.innerHTML = `<option value="">${lang==='ar'?'اختر القسم':'Select Department'}</option>` + deptList.map(d=>`<option value="${d}" ${d===selectedDept?'selected':''}>${d}</option>`).join('');
  if(posSel)  posSel.innerHTML  = `<option value="">${lang==='ar'?'اختر المنصب':'Select Position'}</option>`  + posList.map(p=>`<option value="${p}" ${p===selectedPos?'selected':''}>${p}</option>`).join('');
}

async function resetEmpModal() {
  editingEmpId = null;
...
async function openEditModal(id) {
  ...
  await loadEmpDropdowns(emp.department||'', emp.job_title||'');
  openModal('addEmpModal');
}
...
async function handleSaveEmployee() {
  ...
  const dept  = $('ae_dept_select')?.value;
  const pos   = $('ae_pos_select')?.value;

  if(!first||!last) return toast(lang==='ar'?'أدخل الاسم الأول والأخير':'Enter first and last name','error');
  if(!email)        return toast(lang==='ar'?'أدخل البريد الإلكتروني':'Enter email','error');

  const btn = $('saveEmpBtn');
  setBtn(btn, true, lang==='ar'?'جاري الحفظ...':'Saving...');

  const payload = { first_name:first, last_name:last, email, phone:phone||null, hire_date:hire||null, department:dept||null, job_title:pos||null, status:'active' };

  let error;
  if(editingEmpId) { ({error} = await sb.from('employees').update(payload).eq('id',editingEmpId)); }
  else             { ({error} = await sb.from('employees').insert([payload])); }

  if(error) { setBtn(btn, false); return toast(error.message,'error'); }

  toast(editingEmpId?(lang==='ar'?'تم التعديل ✅':'Updated ✅'):(lang==='ar'?'تم الإضافة ✅':'Added ✅'),'success');
  editingEmpId = null;
...
function openOptionModal(type) {
  ...
  div.id = 'tempOptModal';
  div.innerHTML = `
    <div style="position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,.55);backdrop-filter:blur(6px);z-index:2000;display:flex;align-items:center;justify-content:center;padding:16px">
      <div style="background:var(--surface);border-radius:var(--r-2xl);padding:28px 24px;width:90%;max-width:400px;border:1px solid var(--border);box-shadow:var(--shadow-lg)">
        <div style="font-size:20px;font-weight:800;color:var(--text);margin-bottom:20px">${title}</div>
        ...
        <div style="display:flex;gap:10px;margin-top:10px">
          ...
          <button id="saveOptBtn" onclick="saveNewOption('${type}')" class="primary-btn">${lang==='ar'?'إضافة':'Add'}</button>
        </div>
      </div>
    </div>
  `;
...
async function saveNewOption(type) {
  ...
  const btn = $('saveOptBtn');
  setBtn(btn, true, lang==='ar'?'جاري...':'...');
  const key = type==='dept'?'departments':'positions';
  const {data:existing} = await sb.from('app_settings').select('value').eq('key',key).single();
  const current = existing?.value||'';
  const newValue = current?current+','+name:name;
  const {error} = await sb.from('app_settings').upsert({key,value:newValue});
  if(error) { setBtn(btn, false); return toast(error.message,'error'); }
  toast(lang==='ar'?'تمت الإضافة ✅':'Added ✅','success');
  document.getElementById('tempOptModal').remove();
  const selectedDept = $('ae_dept_select')?.value||'';
  const selectedPos  = $('ae_pos_select')?.value||'';
  await loadEmpDropdowns(selectedDept,selectedPos);
  if(type==='dept'&&$('ae_dept_select')) $('ae_dept_select').value=name;
  if(type==='pos' &&$('ae_pos_select'))  $('ae_pos_select').value=name;
}

async function renderHRSalaries() {
  const {data:emps} = await sb.from('employees').select('*').eq('status','active').order('first_name');
  const items = emps||[];
  if(items.length===0) { $('hrContent').innerHTML=`<div class="empty"><div class="empty-icon">👥</div><div class="empty-title">${lang==='ar'?'لا يوجد موظفون نشطون':'No active employees'}</div></div>`; return; }
  if(!salaryEmpId) { salaryEmpId=items[0].id; selectedEmp=items[0]; }
  else { selectedEmp=items.find(e=>e.id===salaryEmpId)||items[0]; salaryEmpId=selectedEmp.id; }
  ...
  adjustments = adjs||[];
  const netSalary = baseSalary+adjustments.reduce((sum,a)=>{ const isPos=['overtime','bonus','allowance'].includes(a.type); return sum+(isPos?a.amount:-a.amount); },0);
  ...
  $('hrContent').innerHTML=`
    ...
    <div class="card" style="margin-bottom:12px;border:1.5px solid rgba(56,189,248,.2)">
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;padding:12px;background:var(--surface-2);border-radius:12px;margin-bottom:10px">
        ...
        <div style="display:flex;align-items:center;gap:8px">
          ...
          <button id="saveSalBtn" onclick="saveBaseSalary()" style="height:36px;padding:0 14px;background:var(--sky);border:none;border-radius:10px;color:#fff;font-size:12px;font-weight:700;cursor:pointer;font-family:inherit">${lang==='ar'?'حفظ':'Save'}</button>
        </div>
      </div>
    ...
    <div class="card">
      ...
      <button id="addAdjBtn" class="primary-btn" onclick="addAdjustment()">${lang==='ar'?'إضافة':'Add'} ${adjColors[adjType]?.label}</button>
    </div>
  `;
}
...
async function addAdjustment() {
  ...
  const btn = $('addAdjBtn');
  setBtn(btn, true, lang==='ar'?'جاري الإضافة...':'Adding...');
...
function renderHRNotifs() {
  $('hrContent').innerHTML=`
    ...
    <button id="sendNotifBtn" class="primary-btn" onclick="sendNotif()" style="width:100%">${t().send_all}</button>
  `;
}
...
function renderHRExport() {
  $('hrContent').innerHTML=`
    ...
    <div class="card">
      ...
      <button id="exportBtn" class="primary-btn" onclick="exportAttendance()" style="width:100%">${t().generate_download}</button>
    </div>
  `;
}
...
async function exportAttendance() {
  ...
  const btn = $('exportBtn');
  setBtn(btn, true, lang==='ar'?'جاري التحضير...':'Preparing...');
  ...
  if(error) { setBtn(btn, false); return toast(error.message,'error'); }
  if(!data||data.length===0) { setBtn(btn, false); return toast(lang==='ar'?'لا توجد بيانات':'No data found','error'); }
...
async function renderHRTasks() {
  ...
  $('hrContent').innerHTML=`
    ...
    <div class="card" style="margin-bottom:16px">
      ...
      <button id="addTaskBtn" class="primary-btn" onclick="addTask()" style="width:100%;margin-top:12px">📤 ${lang==='ar'?'إرسال التاسك':'Send Task'}</button>
    </div>
  ...
  $('hrContent').innerHTML = `
    ...
    <div class="card" style="margin-bottom:16px">
      ...
      <div style="border:2px dashed var(--border);border-radius:var(--r-lg);padding:24px;text-align:center;cursor:pointer;transition:border-color .2s"
           ...
           ondrop="handleDrop(event,this)" id="dropZone">
      ...
      <div id="uploadProgressContainer" style="display:none;margin-top:12px">
        <div style="display:flex;justify-content:space-between;font-size:12px;color:var(--sub);margin-bottom:4px">
          <span id="uploadStatus">${lang==='ar'?'جاري الرفع...':'Uploading...'}</span>
      ...
      :empFiles.map(f=>{
        ...
        return `<div class="card-sm" style="display:flex;align-items:center;gap:12px">
          ...
          <div style="display:flex;gap:8px;flex-shrink:0">
            <button onclick="downloadFileSecurely('${f.id}','${f.file_path||f.file_url}')" style="background:rgba(56,189,248,.12);border:none;border-radius:8px;width:34px;height:34px;display:flex;align-items:center;justify-content:center;cursor:pointer;font-size:16px">👁</button>
            <button onclick="deleteFile('${f.id}','${f.file_path||f.file_url}')" style="background:rgba(239,68,68,.12);border:none;border-radius:8px;width:34px;height:34px;cursor:pointer;color:var(--red);font-size:14px">✕</button>
          </div>
        </div>`;
      }).join('')}
  `;
...
async function handleFileUpload(input) {
  ...
  if(!FILE_CONFIG.isValid(file.type,file.size)) {
    if(file.size>FILE_CONFIG.MAX_SIZE_MB*1024*1024) return toast(lang==='ar'?`الملف أكبر من ${FILE_CONFIG.MAX_SIZE_MB}MB`:`File exceeds ${FILE_CONFIG.MAX_SIZE_MB}MB`,'error');
    return toast(lang==='ar'?'نوع الملف غير مدعوم':'Invalid file type','error');
  }
  const dz=$('dropZone'),upCont=$('uploadProgressContainer'),upBar=$('uploadBar'),upTxt=$('uploadStatus'),upPct=$('uploadPercent');
  if(upCont) upCont.style.display='block';
  if(dz) dz.style.opacity='0.5';
  let progress=0;
  const progressInterval=setInterval(()=>{ progress+=10; if(upBar) upBar.style.width=Math.min(progress,90)+'%'; if(upPct) upPct.textContent=Math.min(progress,90)+'%'; if(progress>=90) clearInterval(progressInterval); },200);
  try {
    ...
    const {error:dbErr}=await sb.from('employee_files').insert([{employee_id:filesEmpId,file_name:file.name,file_path:fileName,file_type:file.type}]);
    if(dbErr) throw dbErr;
    clearInterval(progressInterval);
    if(upBar) upBar.style.width='100%';
    if(upPct) upPct.textContent='100%';
    if(upTxt) { upTxt.textContent=lang==='ar'?'✅ تم الرفع بنجاح':'✅ Upload Complete'; upTxt.style.color='var(--green)'; }
...
function handleDrop(e,el) {
  ...
  if(file) { const input=$('fileInput'); const dt=new DataTransfer(); dt.items.add(file); input.files=dt.files; handleFileUpload(input); }
}
...
async function downloadFileSecurely(id,pathOrUrl) {
  ...
  try {
    let path=pathOrUrl;
    if(path.startsWith('http')) { const urlObj=new URL(path); const parts=urlObj.pathname.split('/'+FILE_CONFIG.STORAGE_BUCKET+'/'); if(parts[1]) path=decodeURIComponent(parts[1]); else throw new Error('Invalid URL format'); }
    const {data,error}=await sb.storage.from(FILE_CONFIG.STORAGE_BUCKET).createSignedUrl(path,60);
    if(error) throw error;
    window.open(data.signedUrl,'_blank');
  } catch(e) { toast(lang==='ar'?'فشل تحميل الملف':'Failed to download file','error'); }
}

async function deleteFile(id,pathOrUrl) {
  let path=pathOrUrl;
  if(path.startsWith('http')) { try { const urlObj=new URL(path); const parts=urlObj.pathname.split('/'+FILE_CONFIG.STORAGE_BUCKET+'/'); if(parts[1]) path=decodeURIComponent(parts[1]); } catch(e){} }
  showConfirm({
    ...
    onOk:async()=>{
      try {
        if(!path.startsWith('http')) { const {error:storageErr}=await sb.storage.from(FILE_CONFIG.STORAGE_BUCKET).remove([path]); if(storageErr) console.warn('Storage delete warning:',storageErr.message); }
...
async function deleteLeave(id,status,empId,startDate,endDate,leaveType) {
  ...
  showConfirm({
    ...
    msg:isDeductible?(lang==='ar'?`هل تريد إرجاع ${days} يوم للرصيد أم الحذف بدون إرجاع؟`:`Delete and return ${days} days to balance?`):(lang==='ar'?'هل أنت متأكد من الحذف؟':'Are you sure you want to delete this request?'),
    okLabel:isDeductible?(lang==='ar'?`حذف + إرجاع ${days} يوم`:`Delete + Return ${days}d`):(lang==='ar'?'حذف':'Delete'),
    okColor:'var(--red)',
    onOk:async()=>{
      if(isDeductible) { const {data:emp}=await sb.from('employees').select('leave_balance').eq('id',empId).single(); await sb.from('employees').update({leave_balance:(emp?.leave_balance??0)+days}).eq('id',empId); }
...
async function renderPageManager() {
  ...
  $('hrContent').innerHTML=`
    ...
    <div class="card" style="margin-bottom:20px">
      ...
      <div class="form-field"><label class="field-label">${lang==='ar'?'عنوان الصفحة':'Page Title'}</label><input class="form-input" type="text" id="cp_title" placeholder="${lang==='ar'?'مثال: سياسات الشركة':'e.g. Company Policies'}"/></div>
      <div class="form-field"><label class="field-label">${lang==='ar'?'الرابط (Slug)':'Link (Slug)'}</label><input class="form-input" type="text" id="cp_slug" placeholder="${lang==='ar'?'مثال: policies':'e.g. policies'}" style="text-transform:lowercase"/><div style="font-size:11px;color:var(--muted);margin-top:4px">${lang==='ar'?'يجب أن بالإنجليزي وبدون مسافات':'Must be english, no spaces'}</div></div>
      <div class="form-field"><label class="field-label">${lang==='ar'?'المحتوى (HTML مدعوم)':'Content (HTML Supported)'}</label><textarea id="cp_content" class="form-input" placeholder="${lang==='ar'?'اكتب المحتوى هنا...':'Write content here...'}" style="min-height:150px"></textarea></div>
      <button id="savePageBtn" class="primary-btn" onclick="saveCustomPage()" style="width:100%">📤 ${lang==='ar'?'حفظ الصفحة':'Save Page'}</button>
    </div>
      ...
      :pages.map(p=>`
        <div class="card-sm">
          <div style="display:flex;justify-content:space-between;align-items:center">
            <div><div style="font-weight:700;color:var(--text)">${p.title}</div><div style="font-size:11px;color:var(--muted)">/${p.slug}</div></div>
...
async function renderCustomPage(slug) {
  ...
  try {
    ...
    target.innerHTML=`<div style="padding:20px;text-align:center;border:1px solid var(--red);border-radius:12px;color:var(--red)"><h3 style="margin-bottom:10px">⚠️ Error</h3><p>${e.message}</p></div>`;
  }
}
...
async function renderHRSettings() {
  ...
  $('hrContent').innerHTML = `
    ...
    <div class="card" style="margin-bottom:16px">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
        <div class="form-field" style="margin:0"><label class="field-label">Latitude</label><input class="form-input" type="number" step="any" id="setting_lat" value="${office?.latitude||''}"/></div>
        <div class="form-field" style="margin:0"><label class="field-label">Longitude</label><input class="form-input" type="number" step="any" id="setting_lng" value="${office?.longitude||''}"/></div>
    ...
    <button id="saveSettingsBtn" class="primary-btn" onclick="saveSettings()" style="width:100%">💾 ${lang==='ar'?'حفظ الإعدادات':'Save Settings'}</button>
  `;
}
...
async function saveSettings() {
  ...
  const {error} = await sb.from('office_location').update({
