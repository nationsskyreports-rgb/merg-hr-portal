...
/* ═══════════════════ SAFE MODAL ═══════════════════ 
   FIX: Backdrop is a separate absolute-positioned layer,
   so tapping TextInput inside the card doesn't close the modal.
══════════════════════════════════════════════════════ */
    ...
    <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}>
      ...
      >
        <View style={{flex:1, justifyContent:'center', alignItems:'center', padding:20}}>
          ...
          />
          {/* Card - stops touch propagation */}
          <TouchableOpacity
            activeOpacity={1}
            onPress={()=>{}} // swallows touch, prevents backdrop close
            style={{
      ...
      >
        ...
        <View style={{alignItems:'center',marginBottom:36}}>
          <Animated.View style={{opacity:logoOp,transform:[{scale:logoScale}]}}>
            <Image source={LOGO_IMG} style={{width:190,height:120,resizeMode:'contain'}} />
          </Animated.View>
          <Animated.Text style={{
            color:'#94A3B8',fontSize:12,marginTop:8,
        ...
        <Animated.View style={{opacity:cardOp,transform:[{translateY:cardY}]}}>
          ...
          <Text style={{color:'#CBD5E1',fontSize:11,textAlign:'center',marginTop:28,letterSpacing:1}}>
            MERGE HR PORTAL v2.0
          </Text>
        </Animated.View>
      </ScrollView>

      {/* ══ FIXED: Reset Password Modal — backdrop is separate from card ══ */}
      ...
      <View style={{flexDirection:'row',justifyContent:'space-between',alignItems:'center',marginBottom:20,paddingTop:4}}>
        <Animated.View style={{transform:[{translateY:floatAnim}]}}>
          <View style={{flexDirection:'row',alignItems:'center'}}>
            <Image source={LOGO_IMG} style={{width:38,height:26,resizeMode:'contain'}} />
        ...
        <View style={{
          ...
          <View style={{flexDirection:'row',gap:12}}>
            <AppBtn
              ...
              loading={checkingIn} disabled={isClockedIn} onPress={onCheckIn}
              style={{flex:1}}
            />
            <AppBtn
              dark={dark} icon="🚪" label={l.check_out} color={t.red}
              loading={checkingOut} disabled={!isClockedIn} onPress={onCheckOut}
              style={{flex:1}}
            />
          </View>
        </View>
...
/* ═══════════════════ HISTORY (FlatList for performance) ═══════════════════ */
        ...
        >
          ...
          <View style={{flexDirection:'row',gap:12,marginBottom:16}}>
            <View style={{flex:1}}>
              ...
              <CalendarPicker dark={dark} value={startDate} onChange={setStartDate} lang={lang} />
            </View>
            <View style={{flex:1}}>
              <Text style={{
                color:t.sub,fontSize:11,fontWeight:'700',textTransform:'uppercase',
              ...
              <CalendarPicker dark={dark} value={endDate} onChange={setEndDate} lang={lang} />
            </View>
          </View>
          ...
          <AppBtn
            ...
            loading={submitting} onPress={handleSubmit}
            style={{marginTop:20}}
          />
        </ScrollView>
      ):(
...
const NotificationsScreenComp = ({dark,employee,goBack,onRead,lang,setLang}) => {
  ...
  const markAllRead=useCallback(async()=>{
    ...
    for(const u of ur) await supabase.from('notifications').update({is_read:true}).eq('id',u.id);
      ...
      >
        ...
        {tab==='notifications'&&<>
          ...
          <HRCard>
            ...
            />
            <TouchableOpacity
              onPress={sendNotification} disabled={notifSending}
              style={{
                backgroundColor:t.indigo,borderRadius:14,padding:15,
                alignItems:'center',opacity:notifSending?0.5:1,...shadow('md'),
              }}
            >
              {notifSending
...
export default function App() {
  ...
  useEffect(()=>{
    ...
    const{data:{subscription}}=supabase.auth.onAuthStateChange((_e,s)=>{
      setSession(s);
      if(s) setScreen('home');
      else { setScreen('login'); setEmployee(null); }
    });
  ...
  const getUserLoc=async()=>{
    ...
    try{
      ...
      try{
        ...
        return{latitude:loc2.coords.latitude,longitude:loc2.coords.longitude};
      }catch(_){return null;}
    }
  };
  ...
  const renderScreen=()=>{
    switch(screen){
      ...
      case 'history':
        return <HistoryScreenComp dark={darkMode} employee={employee} goBack={()=>nav('home')} lang={lang} setLang={handleSetLang} />;
      case 'map':
        return <MyLocationScreenComp dark={darkMode} goBack={()=>nav('home')} lang={lang} setLang={handleSetLang} />;
      case 'leave':
        return <LeaveRequestScreenComp dark={darkMode} employee={employee} goBack={()=>nav('home')} lang={lang} setLang={handleSetLang} />;
      case 'notifications':
        return <NotificationsScreenComp dark={darkMode} employee={employee} goBack={()=>nav('home')} onRead={fetchUnread} lang={lang} setLang={handleSetLang} />;
      case 'profile':
        return <ProfileScreenComp dark={darkMode} employee={employee} goBack={()=>nav('home')} setDarkMode={setDarkMode} onChangePassword={()=>nav('change_password')} lang={lang} setLang={handleSetLang} />;
      case 'change_password':
        return <ChangePasswordScreenComp dark={darkMode} goBack={()=>nav('home')} lang={lang} setLang={handleSetLang} />;
      case 'hr_dashboard':
        return <HRDashboardScreenComp dark={darkMode} goBack={()=>nav('home')} lang={lang} />;
      default:
        return null;
    }
  };
