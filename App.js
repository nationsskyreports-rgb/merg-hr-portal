...
/* ══════════════════════════════════════════════════════
   CROSS-PLATFORM ALERT — works on web + mobile
══════════════════════════════════════════════════════ */
...
const HomeScreen = ({ dark, employee, isClockedIn, checkingIn, checkingOut, unreadCount, onCheckIn, onCheckOut, onNav, lang, setLang, onRefresh }) => {
  ...
  const handleLogout = () => {
    crossAlert(l.logout_confirm, l.logout_msg, [
      ...
      { text: l.sign_out, style: 'destructive', onPress: async () => {
        try { await supabase.auth.signOut(); } catch (_) {}
        finally { setScreen('login'); setEmployee(null); setIsClockedIn(false); setUnreadCount(0); }
      }},
    ]);
  };
...
export default function App() {
  ...
  const handleCheckIn = async () => {
    ...
    try {
      ...
      if (dist > allowed) {
        crossAlert(L[lang].out_of_range,
